      ...
      <div className="bg-white shadow-sm border-b border-gray-50">
        <div className="w-11/12 mx-auto px-6 py-3">
          <div className="flex items-center justify-between">
            <p className="text-2xl text-gray-700 font-montserrat font-light tracking-wide">Collector's Scrapbook</p>
            <div>
              {isAuthed ? (
                <button onClick={() => signOut()} className="px-3 py-1.5 rounded bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm border border-gray-200 transition-colors">Sign out</button>
              ) : (
                <button onClick={() => signIn('google')} className="px-3 py-1.5 rounded bg-blue-500 hover:bg-blue-600 text-white text-sm transition-colors">Sign in with Google</button>
              )}
            </div>
          </div>
        </div>
