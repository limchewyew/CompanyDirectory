...
export async function addToCollection(userEmail: string, companyId: string | number): Promise<void> {
  const sheets = await getSheets();
  const tab = 'UserCollections';
...
export async function getUserCollection(userEmail: string): Promise<CollectionItem[]> {
  const sheets = await getSheets();
  const tab = 'UserCollections';
  ...
  return data
    .filter(row => row[1] === userEmail)  // Filter by userEmail (userId)
    .map(row => ({
      id: row[0],
      userId: row[1],
      companyId: row[2],
...
export async function isCompanyCollected(userEmail: string, companyId: string | number): Promise<boolean> {
  const collection = await getUserCollection(userEmail);
  return collection.some(item => String(item.companyId) === String(companyId));
}
